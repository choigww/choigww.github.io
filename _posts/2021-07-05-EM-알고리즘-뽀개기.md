...
- 가정
  ...
  - $$
    P(X|\theta)=\Sigma_ZP(X,Z|\theta)\rightarrow lnP(X|\theta)=ln\{\Sigma_ZP(X,Z|\theta)\}
    $$
...
- 우리가 알고 싶은 것
  ...
  - $Z,\theta$
    - 최적화하고자 하는 계산식 $P(X|\theta)=\Sigma_Z P(X,Z|\theta)$
...
- $ln\{\Sigma_Z q(Z)\frac{P(X,Z|\theta)}{q(Z)}\}\geq \Sigma_Zq(Z)ln\frac{P(X,Z|\theta)}{q(Z)}$
...
- The other storyline is:
  ...
  - 여기서 두번째 항은 매우 특별한 식을 의미함!
    $$
    KL(q(Z)||P(Z|X,\theta))=\Sigma_Z\{q(Z)\text{ln}\frac{q(Z)}{p(Z|X,\theta)}\}
    $$

    - [쿨백-라이블러 다이버전스](https://hyunw.kim/blog/2017/10/27/KL_divergence.html)(KL divergence): $KL(P||Q)=\sum_iP(i)\text{ln}\frac{P(i)}{Q(i)}$
...
- $L(\theta,q)$의 두번째 항인 **KL Divergence**는 $q(Z)$를 **어떻게 업데이트 해 나가야 할 지를 알려준다**
  ...
  - $L(\theta,q)$의 두번째 항은 $L(\theta,q)$ 최대화를 위해 **최소화**될 수 있다
    ...
    - (정보이론 관점) $q(Z)$와 $P(Z|X,\theta)$ 간의 차이가 작아지면 실제 log likelihood에 가깝게 다가갈 수 있다
  - 최적화된 $q$에 대한 lower bound는 다음과 같이 업데이트:
    - $Q(\theta,q^t)=E_{q^t}\text{ln}P(X,Z|\theta^t)+H(q^t)$
- more tight lower bound (=maximized $Q(\theta,q)$)를 얻기 위해 $\theta$ 최적화 진행
  - $\theta^{t+1}=argmax_\theta Q(\theta,q^t)=argmax_\theta E_{q^t (Z)}\text{ln}P(X,Z|\theta)$
    - $q^t(Z) \rightarrow$  t 시점에 계산된 잠재변수의 분포 모수
    - $\text{ln}P(X,Z|\theta)\rightarrow$  t+1 시점에 업데이트 최적화 된 log likelihood 모수
...
- EM Algorithm
  ...
  - $P(X|\theta)=\Sigma_ZP(x,Z|\theta)\rightarrow\text{ln}P(X|\theta)=\text{ln}\{\Sigma_ZP(X,Z|\theta)\}$
...
$$
q^{t+1}(z)=argmax_q Q(\theta^t,q)=argmax_q L(\theta^t,q)=argmin KL(q||P(Z|X,\theta^t))
\\
...
\\
=\frac{\pi_k N(x|\mu_k,\Sigma_k)}{\Sigma^K_{j=1} \pi_j N(x|\mu_j,\Sigma_j)}
$$
...
일반적인 Maximum Likelihood Estimation (최대우도추정법,  MLE)와 동일한 최적화 방법
$$
\frac{d}{d \mu_k}\text{ln}P(X|\pi,\mu,\Sigma)=0, \ \frac{d}{d \Sigma_k}\text{ln}P(X|\pi,\mu,\Sigma)=0, \ \frac{d}{d \mu_k}\text{ln}P(X|\pi,\mu,\Sigma)+\lambda(\Sigma^K_{k=1}\pi_k-1)=0
$$
